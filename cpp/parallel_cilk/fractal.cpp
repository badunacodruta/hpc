...
int Fractal::init(int argc, char *argv[]) {
    ...
    for (i = 1; i < argc; i++) {
		...
		if (strcmp(argv[i], "--write_type") == 0 && i + 1 < argc) {
			if (strcmp(argv[i+1], "onGoing") == 0){
				writeMode = ON_GOING;
            }
            else if (strcmp(argv[i+1], "atEnd") == 0){
	            writeMode = AT_END;
            }
			...
			cout << "Write mode: " << writeMode << endl;
		}
...
void Fractal::slaveComplex(){
	...
	while(!jobsToProcess.empty ()){
		...
		if (writeMode != ON_GOING){
			rgb = NULL;
			rgb = (RGB**) malloc ( sizeof(RGB*) * height);
			if (rgb == NULL){
				 exit (0);
			}
		}
		...
		FILE *ff = f;
		double ccount = count;
        int ccolorNo = colorNo;
        double ccolorPercent = colorPercent;
        long double zzr = zr, zzi = zi, ccr = cr, cci = ci;
        long double rrsquared = rsquared, iisquared = isquared;
        int *ccolor1 = color1, *ccolor2 = color2;
        int nnoCol = noCol;

		int j,i;
		int wwidth = width;
        int hheight = height;
		int mmax_iterations = max_iterations;
		int wwriteMode = writeMode;

		for (y = 0; y < hheight; y++) {
			if (writeMode != ON_GOING){
				rgb[y] = NULL;
				rgb[y] = (RGB*) malloc (sizeof(RGB) * wwidth);
				if (rgb[y] == NULL){
					exit (0);
				}
			}
		...
		for (i = 0; i < hheight; i++) {
				for (j = 0; j < wwidth; j++) {
					...
					zzr = 0.0;
					zzi = 0.0;
					ccr = left + j * (right - left) / width;

					cci = top + i * (bottom - top) / height;
					rrsquared = zzr * zzr;
					iisquared = zzi * zzi;

					for (ccount = 0;
							rrsquared + iisquared <= 4.0 && ccount < mmax_iterations;
							ccount++) {
						zzi = zzr * zzi * 2;
						zzi += cci;
						zzr = rrsquared - iisquared;
						zzr += ccr;
					...
					if (wwriteMode == ON_GOING){
							...
							// Else, if we are outside the fractal, we need to choose the gradient color.
								...
								fprintf ( ff, "%d %d %d ",
									interpolate(ccolor1[0], ccolor2[0], ccolorPercent),
						...
						// Else, if we are outside the fractal, we need to choose the gradient color.
							...
							RGB color;
							color.r = interpolate(ccolor1[0], ccolor2[0], ccolorPercent);
							color.g = interpolate(ccolor1[1], ccolor2[1], ccolorPercent);
							color.b = interpolate(ccolor1[2], ccolor2[2], ccolorPercent);
							rgb[i][j] = color;
						}
					}
				}
		...
		if (writeMode == AT_END){
			...
			for (i = 0; i < height; i++) {
				for (j = 0; j < width; j++) {
					fprintf ( ff, "%d %d %d ", rgb[i][j].r, rgb[i][j].g, rgb[i][j].b);
				}
